...
from models.qwenvl import load_qwenvl
...
def forward(messages, model, processor, target_tokens):
    ...
    image_start_idx = torch.where(inputs.input_ids == processor.tokenizer.encode("<|vision_start|>")[0])[1]
    image_end_idx = torch.where(inputs.input_ids == processor.tokenizer.encode("<|vision_end|>")[0])[1]
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(data_path, "qwenvl")

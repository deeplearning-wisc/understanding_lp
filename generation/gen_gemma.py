...
def forward(messages, model, processor, target_tokens):
    ...
    image_start_idx = torch.where(inputs.input_ids == 255999)[1] - 1
    image_end_idx = torch.where(inputs.input_ids == 256000)[1]
...
if __name__ == "__main__":
    ...
    model, processor = load_gemma(model_name)
    model.eval()
    ...
    output_dir = os.path.join(data_path, model_name.split("/")[-1])

...
def forward(messages, image, model, processor, target_tokens):
    ...
    image_tokens_idx = torch.where(inputs.input_ids == processor.tokenizer.encode("<|image|>", add_special_tokens=False)[0])[1]
    ...
    if image is not None:
        image_start_idx = image_tokens_idx[0]
        image_end_idx = image_tokens_idx[-1]
        # print(f"image token num: {image_end_idx - image_start_idx + 1}")
...
def get_generation_pattern(image, instruction, model, processor, target_tokens):
    ...
    last_token_hidden_states_vl, last_token_visual_attention_scores_vl, target_tokens_probs_vl = forward(messages, image, model, processor, target_tokens)    
    last_token_hidden_states_l, last_token_visual_attention_scores_l, target_tokens_probs_l = forward(messages_without_image, None, model, processor, target_tokens)
    
    self_attn_layers = [0, 1, 2, 3, 5, 6, 7, 8, 10, 11, 12, 13, 15, 16, 17, 18, 20, 21, 22, 23, 25, 26, 27, 28, 30, 31, 32, 33, 35, 36, 37, 38, 40]
    
    last_token_hidden_states_vl = [last_token_hidden_states_vl[i] for i in self_attn_layers]
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(data_path, "llama")
    ...
    if args.test:
      ...
      print(hidden_states_vl[0][0][:5], hidden_states_vl[0][0][-5:])
      print(hidden_states_l[0][0][:5], hidden_states_l[0][0][-5:])
      exit()
